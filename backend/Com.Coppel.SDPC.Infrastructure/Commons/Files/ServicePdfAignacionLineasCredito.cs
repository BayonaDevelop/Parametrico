...
public class ServicePdfAignacionLineasCredito : IServicePdfAignacionLineasCredito
{
	...
	private void CreateTable(Document document)
	{
		...
		if (!_request.NewData)
		{
			_tablaDatos.AddHeaderCell(new Cell(1, _table.Columns.Count).Add(_tablaFechas).SetBorder(iText.Layout.Borders.Border.NO_BORDER));
		...
		{
			table.AddHeaderCell(new Cell(1, _table.Columns.Count).Add(_tablaFechas).SetBorder(iText.Layout.Borders.Border.NO_BORDER));
	...
	private Image InsertLogo()
	{
		...
		{
			// Convert the image to a byte array (e.g., PNG format)
